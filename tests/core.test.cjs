...
test("PDF plan preserves seven days, exercise order, reps and sets", () => {
  assert.deepEqual(
    PLAN.map((p) => p.exercises.length),
    [6, 6, 6, 2, 7, 6, 0],
  );
  assert.deepEqual(
    PLAN[1].exercises.map((e) => e.name),
    [
      "Squats",
      "Reverse lunges",
      "Glute bridges",
...
test("separates exercise from rest; rest expiry requires explicit start", () => {
  const s = new Session(0, {}, start);
  s.tick(start + 12000);
  s.complete(true, start + 12000);
  assert.equal(s.activeMs, 12000);
  assert.equal(s.index, 1);
  assert.equal(s.phase, "rest");
  s.tick(start + 100000);
  assert.equal(s.restMs, 45000);
  assert.equal(s.activeMs, 12000);
  assert.equal(s.phase, "ready");
  s.tick(start + 200000);
  assert.equal(s.activeMs, 12000);
  s.startNext(start + 200000);
  s.tick(start + 205000);
  assert.equal(s.activeMs, 17000);
});
test("timed exercise clamps at target, prevents double completion", () => {
  const s = new Session(5, {}, start);
  s.tick(start + 90000);
  assert.equal(s.activeMs, 45000);
  assert.equal(s.phase, "setDone");
  s.complete(true, start + 91000);
  s.complete(true, start + 92000);
  assert.equal(s.index, 1);
...
test("pausing and restoring never count time away", () => {
  ...
  s.pause(start + 8000);
  s.tick(start + 18000);
  assert.equal(s.activeMs, 8000);
  s.resume(start + 20000);
  s.tick(start + 25000);
  assert.equal(s.activeMs, 13000);
  const copy = Session.restore(JSON.parse(JSON.stringify(s)), start + 80000);
  assert.equal(copy.phase, "paused");
  copy.tick(start + 90000);
  assert.equal(copy.activeMs, 13000);
  copy.resume(start + 100000);
  copy.tick(start + 103000);
  assert.equal(copy.activeMs, 16000);
});
test("mid-set rest retains work time and does not complete a set", () => {
  const s = new Session(5, {}, start);
  s.breakNow(start + 10000);
  assert.equal(s.logs.length, 0);
  assert.equal(s.stepMs, 10000);
  s.startNext(start + 20000);
  s.tick(start + 25000);
  assert.equal(s.stepMs, 15000);
  assert.equal(s.restMs, 10000);
});
test("full session has ordered logs and an accurate completion record", () => {
  ...
  for (let i = 0; i < 18; i++) {
    now += 60000;
    s.complete(false, now);
  }
  assert.equal(s.phase, "finished");
  assert.equal(s.logs.length, 18);
  assert.equal(s.logs[3].name, "Pike push-ups");
  assert.equal(s.logs[3].set, 1);
...
test("streak counts unique completed dates, preserves Saturday, ignores partial sessions", () => {
  const h = ["2026-09-03", "2026-09-04", "2026-09-06"].map((date) => ({
    date,
    completed: true,
    kind: "workout",
  }));
  assert.equal(streak(h, "2026-09-06"), 3);
  assert.equal(streak(h, "2026-09-07"), 3);
  h.push(h[2]);
  assert.equal(streak(h, "2026-09-07"), 3);
  h.push({ date: "2026-09-07", completed: false });
  assert.equal(streak(h, "2026-09-07"), 3);
...
test("recovery completion counts and a missed recovery breaks the streak", () => {
  const h = [
    { date: "2026-09-01", completed: true, kind: "workout" },
    { date: "2026-09-03", completed: true, kind: "workout" },
  ];
  assert.equal(streak(h, "2026-09-03"), 1);
  h.push({ date: "2026-09-02", completed: true, kind: "recovery" });
  assert.equal(streak(h, "2026-09-03"), 3);
});
