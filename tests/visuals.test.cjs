...
const names = [
  ...new Set(PLAN.flatMap((day) => day.exercises.map((e) => e.name))),
];
test("every planned movement has a specific visual and a complete guide", () => {
  for (const name of names) {
    ...
    assert.equal(
      guides[name].steps.length,
      3,
      `${name}: expected three readable steps`,
    );
    assert.ok(
      guides[name].cue && guides[name].equipment && guides[name].avoid,
      `${name}: incomplete cues`,
    );
    ...
    for (const compact of [true, false]) {
      ...
      assert.doesNotMatch(
        svg,
        /<image\b|<script\b|onload=|(?:href|src)=["\']https?:\/\//i,
        `${name}: SVG must be self contained`,
      );
    }
  }
});
test("native and desktop asset allowlists include both visual libraries before app initialization", () => {
  ...
  const native = fs.readFileSync(
    "app/src/main/java/com/sixpack/trainer/MainActivity.java",
    "utf8",
  );
  ...
  for (const file of [
    ...
    assert.ok(native.includes(`"${file}"`));
    assert.ok(preview.includes(`'${file}'`));
  }
});
